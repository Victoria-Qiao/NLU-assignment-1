...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		best_lr = 0.5
		best_steps = 0
		rnn = RNN(vocab_size=vocab_size, hidden_dims=best_hdim, out_vocab_size=vocab_size)
		run_loss = rnn.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, learning_rate=best_lr, back_steps=best_steps)
	...
	if mode == "train-np":
		...
		best_loss = rnn.train_np(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, learning_rate=lr, back_steps=lookback)
	...
	if mode == "predict-lm":
		...
		U,V,W = np.load(rnn_folder + "/rnn_u_50.npy"), np.load(rnn_folder + "/rnn_v_50.npy"), np.load(rnn_folder + "/rnn_w_50.npy")
		vocab_size = len(V[0])
		hdim = len(U[0])
