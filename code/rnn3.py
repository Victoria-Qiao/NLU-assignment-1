...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			net_in = self.V[:,x[t]] + self.U.dot(s[t-1])
			s[t] = 1./(1 + np.exp(-net_in))
			net_out = self.W.dot(s[t])
			y[t] = np.exp(net_out)/(np.exp(net_out).sum())
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			d_one_hot = np.array([1 if i == d[t] else 0 for i in range(self.out_vocab_size)]).reshape((-1,1))
			theta_out = d_one_hot - y[t].reshape((-1,1))
			self.deltaW += theta_out.dot(s[t].reshape((1, -1)))

			x_one_hot = np.array([1 if i == x[t] else 0 for i in range(self.vocab_size)]).reshape((1, -1))
			softmax_gradient = (np.multiply(s[t], 1-s[t])).reshape(-1, 1)
			theta_in = np.multiply((self.W).T.dot(theta_out), softmax_gradient)
			self.deltaV += theta_in.dot(x_one_hot)

			self.deltaU += theta_in.dot(s[t-1].reshape((1,-1)))
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		d_one_hot = np.array([1 if i == d[0] else 0 for i in range(self.out_vocab_size)]).reshape((-1, 1))
		x_one_hot = np.array([1 if i == x[-1] else 0 for i in range(self.vocab_size)]).reshape((1, -1))
		theta_out = d_one_hot - y[-1].reshape((-1, 1))
		softmax_gradient = (np.multiply(s[-2], 1 - s[-2])).reshape(-1, 1)
		theta_in = np.multiply(self.W.T.dot(theta_out), softmax_gradient).reshape((-1, 1))

		self.deltaW += theta_out.dot(s[-2].reshape((1, -1)))
		self.deltaV += theta_in.dot(x_one_hot)
		self.deltaU += theta_in.reshape(-1, 1).dot(s[(-3)%(len(s))].reshape((1, -1)))
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for i in range(steps):
				id = t-1-i
				softmax_gradient = (np.multiply(s[id], 1 - s[id])).reshape(-1, 1)
				x_one_hot = np.array([1 if i == x[id] else 0 for i in range(self.vocab_size)]).reshape((1, -1))
				theta_in = np.multiply(self.U.T.dot(theta_in), softmax_gradient)
				self.deltaV += theta_in.dot(x_one_hot)
				self.deltaU += theta_in.reshape(-1, 1).dot(s[id-1].reshape((1, -1)))
	...
	def acc_deltas_bptt_np(self, x, d, y, s, steps):
		...
		for i in range(steps):
			...
			if (2-id)>len(s):
				break
			x_one_hot = np.array([1 if i == x[id] else 0 for i in range(self.vocab_size)]).reshape((1, -1))
			softmax_gradient = (np.multiply(s[id-1], 1 - s[id-1])).reshape(-1, 1)
			theta_in = np.multiply(self.U.T.dot(theta_in), softmax_gradient)
			self.deltaV += theta_in.dot(x_one_hot)
			self.deltaU += theta_in.reshape(-1, 1).dot(s[id-2].reshape((1, -1)))
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		for epoch in range(epochs):
			...
			if abs(prev_loss - loss) < min_change:
				...
				min_change_count = 0
			if min_change_count > 2: # smaller than min_change for consective three times, then stop
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		for dim in hdim:
			for rate in lr:
				for step in lookback:
					np.random.seed(2018)
					rnn = RNN(vocab_size=vocab_size, hidden_dims=dim, out_vocab_size=vocab_size)
					loss = rnn.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, learning_rate=rate, back_steps=step)
					if loss < best_loss:
						best_loss = loss
						best_hdim = dim
						best_lr = rate
						best_steps = step
		print('Best loss is {0}'.format(best_loss))
		print('Best hdim is {0}'.format(best_hdim))
		print('Best lr is {0}'.format(best_lr))
		print('Best lookback is {0}'.format(best_steps))
		'''
		# Question 2b
		# best_hdim = 50
		# best_lr = 0.5
	...
	if mode == "train-np":
		...
		loss, acc = rnn.train_np(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, learning_rate=lr,
								 back_steps=lookback)
	...
	if mode == "part-of-speech":
		'''
		Question 5:
        convert each sentence in the training data file into a sequence of tags of part of speech
        save the generated sequences of tags of part of speech into files
        you can find 'trag_train.txt' in data_folder after finishing running code
        '''
		data_folder = sys.argv[2]
		data_file = sys.argv[3]
		# data_file = data_folder + '/wiki-train.txt'
		tag_train = pd.read_csv(data_folder+data_file, sep='\t')
		cnt = 0
			...
			for line in f:
				...
				for item in tag_list:
					...
					if item[0] == 'NN':
						...
						continue
					new_tag_list += item[1] + ' '
				tag_train['sentence'][cnt - 1] = new_tag_list
				cnt += 1

		tag_train.to_csv(data_folder + '/trag_train.txt', sep='\t', index=False)

	if mode == "train-np-part-of-speech":
		'''
		Question 5
		This is to run and get the accuracy of number prediction with the generated tags of part of speech
		'''
		data_folder = sys.argv[2]
		...
		# get the data set vocabulary
		vocab = pd.read_table(data_folder + "/tag.wiki.txt", header=None, sep="\s+", index_col=0)
		num_to_word = dict(enumerate(vocab.index[:vocab_size]))
		word_to_num = invert_dict(num_to_word)

		# load training data
		sents = load_np_dataset(data_folder + '/tag_train.txt')
		S_train = docs_to_indices(sents, word_to_num, 0, 0)
		X_train, D_train = seqs_to_npXY(S_train)
		...
		# load development data
		sents = load_np_dataset(data_folder + '/tag_dev.txt')
		S_dev = docs_to_indices(sents, word_to_num, 0, 0)
		X_dev, D_dev = seqs_to_npXY(S_dev)
		...
		rnn = RNN(vocab_size=vocab_size, hidden_dims=hdim, out_vocab_size=2)
		best_loss, acc_vec = rnn.train_np(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, learning_rate=lr,
										  back_steps=lookback)